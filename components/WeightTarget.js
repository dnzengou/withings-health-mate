...
export default class WeightTarget extends React.PureComponent<WeightTargetProps, WeightTargetState> {
    ...
    makeHeaderVisible = () => Animated.timing(this.state.visibleHeader, {
      duration: 300,
      toValue: 1,
      useNativeDriver: true,
    }).start();

    makeHeaderInvisible = () => Animated.timing(this.state.visibleHeader, {
      duration: 300,
      toValue: 0,
      useNativeDriver: true,
    }).start();
    ...
    render() {
      const { from, to } = this;
      const { y, visibleModal, visibleHeader } = this.state;
      ...
      const translateHeader = visibleHeader.interpolate({
        inputRange: [0, 1],
        outputRange: [-64, 0],
      });
      return (
